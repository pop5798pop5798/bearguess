...
namespace SITW.Models.Repository
{
    public class BetsRepository
    {
        ...
        public async System.Threading.Tasks.Task<List<BetListDto>> BetsLiveByUserID(string UserID)
        {
            List<BetListDto> BetList = new List<BetListDto>();

            HttpResponseMessage response = await client.GetAsync("api/LiveBetsByUserID?userID=" + UserID);
            if (response.IsSuccessStatusCode)
            {
                BetList = await response.Content.ReadAsAsync<List<BetListDto>>();
            }
