function CGame(oData) {
    ...
    this._endReelAnimation = function () {
        ...
        if (_aWinningLine.length > 0) {
            ...
            if (_auto == 1 && _iMoney >= _iTotBet) {
                s_oGame.onSpinauto();
            } else {
                _oInterface._onSpinstopdown();
            }
    ...
    this.addLine = function () {
        ...
        if (iNewTotalBet > _iMoney) {
            ...
            if (_iMoney < MAX_BET * NUM_PAYLINES) {
                _oInterface.enableSpinOne();

            } else {
                _oInterface.enableSpin();
                _oInterface.enableReSpin();
            }
        }
       
    };

    this.UnaddLine = function () {
        if (_iLastLineActive == MIN_PAYLINES) {
            _iLastLineActive = 1;
        } else {
            _iLastLineActive--;
        }
    ...
    this.changeCoinBet = function () {
        var iNewBet = Math.floor((_iCurBet + BET_UP) * 100) / 100;
        var iNewTotalBet;
    ...
    this.UnchangeCoinBet = function () {
        var iNewBet = Math.floor((_iCurBet + BET_UP) * 100) / 100;
        var iNewTotalBet;

        if (iNewBet <= MIN_BET * 2) {
            _iCurBet = MIN_BET;
            _iTotBet = _iCurBet * _iLastLineActive;
            _iTotBet = parseFloat(_iTotBet.toFixed(2));
    ...
    this.onSpin = function () {
        ...
        if (_iTotBet >= 1500 && _iTotBet < 3000) {
            _max = SLOT_CASH * 0.1 + _iTotBet;
        } else if (_iTotBet >= 3000 && _iTotBet < 4500) {
            _max = SLOT_CASH * 0.2 + _iTotBet;
        } else if (_iTotBet >= 4500 && _iTotBet < 6000) {
            _max = SLOT_CASH * 0.4 + _iTotBet;
        } else if (_iTotBet >= 6000 && _iTotBet < 9000) {
            _max = SLOT_CASH * 0.7 + _iTotBet;
        } else if (_iTotBet >= 9000) {
            _max = SLOT_CASH + _iTotBet;
        }
        ...
        if (SLOT_CASH < MIN_WIN) {
            ...
            if (iRandSpin > WIN_OCCURRENCE) {
                ...
                do {
                    var bRet = this.generateFinalSymbols();
                    _win = _iTotWin * _iCurBet;

                } while (bRet === false || _iTotWin * _iCurBet > _max);
        ...
        var url = location.protocol + "//" + location.host + "/Html5/SlottoUserData";
        $.ajax({
            url: url,
            type: "POST",
            data: { money: _iTotBet, id: _iSlomid, win: _win },
            dataType: "json",
            success: function (msg) {
                SLOT_CASH = msg.slotm.baseCash;
                _TolBet -= _win;
            }
    ...
    this.onExit = function () {
        ...
        var url = location.protocol + "//" + location.host + "/Html5/SlottoLogin";
        $.ajax({
            url: url,
            type: "GET",
            dataType: "json",
            success: function (msg) {
                ...
                } else if (msg.loginUser == 1) {
                    mainLoad(msg.usermoney, msg.slotm.slot_cash, msg.slotm.id);
                }
