function CInterface(iCurBet,iTotBet,iMoney){
    ...
    this._init = function(iCurBet,iTotBet,iMoney){
        ...
        _oSpinBut = new CTextButtonS(1026 + (oSprite.width / 2), 595, oSprite, TEXT_SPIN, FONT_GAME, "#ffffff", 30, s_oStage, 0);
        _oSpinBut.addEventListener(ON_MOUSE_DOWN, this._onSpindown, this);
        _oSpinBut.addEventListener(ON_MOUSE_UP, this._onSpinup, this);

        oSprite = s_oSpriteLibrary.getSprite('spin_but');
        _reoSpinBut = new CTextButtonS(1026 + (oSprite.width / 2), 595, oSprite, '金額不足', FONT_GAME, "#ffffff", 30, s_oStage, 0);
        _reoSpinBut.disable();
        _reoSpinBut.setVisible(false);
        

        oSprite = s_oSpriteLibrary.getSprite('spin_but');
        _oSpinstopBut = new CTextButton(1026 + (oSprite.width / 2), 595, oSprite, "停止", FONT_GAME, "#ffffff", 30, s_oStage,0);
        _oSpinstopBut.addEventListener(ON_MOUSE_DOWN, this._onSpinstopdown, this);
        _oSpinstopBut.setVisible(false);
        
        oSprite = s_oSpriteLibrary.getSprite('info_but');
        _oInfoBut = new CTextButton(296 + (oSprite.width/2),595,oSprite,TEXT_INFO,FONT_GAME,"#ffffff",30,s_oStage,0);        
        _oInfoBut.addEventListener(ON_MOUSE_UP, this._onInfo, this);
        
        oSprite = s_oSpriteLibrary.getSprite('but_lines_bg');
        _oAddLineBut = new CTextButton(477 + (oSprite.width/2),595,oSprite,TEXT_LINES,FONT_GAME,"#ffffff",18,s_oStage,0);
        _oAddLineBut.disable();

        oSprite = s_oSpriteLibrary.getSprite('but_lines_bg_right');
        _oAddLineBut = new CTextButtonB(477 + (oSprite.width / 2) + 95, 595, oSprite, "+", FONT_GAME, "#ffffff", 30, s_oStage, 0);
        _oAddLineBut.addEventListener(ON_MOUSE_UP, this._onAddLine, this);

        oSprite = s_oSpriteLibrary.getSprite('but_lines_bg_left');
        _oAddLineBut = new CTextButtonB(440 + (oSprite.width / 2), 595, oSprite, "-", FONT_GAME, "#ffffff", 30, s_oStage, 0);
        _oAddLineBut.addEventListener(ON_MOUSE_UP, this._onUnAddLine, this);

        
        oSprite = s_oSpriteLibrary.getSprite('coin_but');
        _oBetCoinBut = new CTextButton(661 + (oSprite.width/2),595,oSprite,TEXT_COIN,FONT_GAME,"#ffffff",18,s_oStage,0);
        //_oBetCoinBut.addEventListener(ON_MOUSE_UP, this._onBet, this);
        _oBetCoinBut.disable();
        oSpriteadd = s_oSpriteLibrary.getSprite('coin_but_right');
        _oBetCoinADDBut = new CTextButtonB(663 + (oSprite.width / 2) + 66, 595, oSpriteadd, "+", FONT_GAME, "#ffffff", 30, s_oStage, 0);
        
        _oBetCoinADDBut.addEventListener(ON_MOUSE_UP, this._onBet, this);

        oSprite = s_oSpriteLibrary.getSprite('coin_but_left');
        _oBetCoinADDBut = new CTextButtonB(660 + (oSprite.width / 2) - 39, 595, oSprite, "-", FONT_GAME, "#ffffff", 30, s_oStage, 0);

        _oBetCoinADDBut.addEventListener(ON_MOUSE_UP, this._onUnaddBet, this);
        
        oSprite = s_oSpriteLibrary.getSprite('but_maxbet_bg');
        _oMaxBetBut = new CTextButton(805 + (oSprite.width / 2), 575, oSprite, TEXT_MAX_BET, FONT_GAME, "#ffffff", 22, s_oStage,3);       
        _oMaxBetBut.addEventListener(ON_MOUSE_UP, this._onMaxBet, this);

        oSprite = s_oSpriteLibrary.getSprite('but_maxbet_bg');
        _oMinBetBut = new CTextButton(805 + (oSprite.width / 2), 575 + 40, oSprite, TEXT_MIN_BET, FONT_GAME, "#ffffff", 22, s_oStage,3);
        _oMinBetBut.addEventListener(ON_MOUSE_UP, this._onMinBet, this);

        _oMoneyText = new createjs.Text(TEXT_MONEY + "\n" + TEXT_CURRENCY + iMoney.toFixed(2),"24px "+FONT_GAME, "#ffde00");
        _oMoneyText.x = 408;
        _oMoneyText.y = 14;
        _oMoneyText.textAlign = "center";
        s_oStage.addChild(_oMoneyText);
        
        _oNumLinesText = new createjs.Text(MIN_PAYLINES ,"26px "+FONT_GAME, "#ffffff");
        ...
        _oTotalBetText = new createjs.Text(TEXT_BET +": "+iTotBet.toFixed(0),"26px "+FONT_GAME, "#ffffff");
    ...
    this.refreshMoney = function(iMoney){
        _oMoneyText.text = TEXT_MONEY + "\n" + TEXT_CURRENCY +iMoney.toFixed(0);
    };
    ...
    this.refreshWinText = function(iWin){
        _oWinText.text = TEXT_WIN + " " + TEXT_CURRENCY + iWin.toFixed(0);
    };
    ...
    this.enableReSpin = function () {
        _reoSpinBut.setVisible(false);
    };

    this.disableReSpin = function () {
        _reoSpinBut.setVisible(true);
    };
    ...
    this.disableSpinMax = function () {
        /*_oMaxBetBut.addEventListener(ON_MOUSE_UP, function () {
            _oMaxBetBut.disable();
            alert('金額不足');
        }, this);*/
       // _oSpinBut.disable();
    ...
    this._onSpindown = function () {
        last = new Date();
    };

    this._onSpinup = function () {
        now = new Date();
        if (now - last > 1 * 1000) {
            _oSpinstopBut.setVisible(true);
    ...
    this._onUnaddBet = function () {
        s_oGame.UnchangeCoinBet();
    };
