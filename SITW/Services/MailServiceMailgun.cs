...
namespace SITW.Services
{
    public class MailServiceMailgun
    {
        ...
        */
        ...
        /// <param name="mobile">接收人之郵件地址。格式為: abc@yahoo.com.tw。多筆接收人時，請以半形逗點隔開( , )，abc@yahoo.com.tw,def@yahoo.com.tw。</param>
        /// <returns>true:傳送成功；false:傳送失敗</returns>
        public bool ChangeSend(string subject, string content, string mailto,string url)
        {
            bool success = false;
            try
            {
                ...
                if(url != "")
                {
                    var clientall = new WebClient();
                    clientall.DownloadFile(url, HttpContext.Current.Server.MapPath("~/App_Data/") + "活動領獎單.pdf");
                    request.AddFile("attachment", HttpContext.Current.Server.MapPath("~/App_Data/") + "活動領獎單.pdf");
        ...
        /// <param name="mobile">接收人之郵件地址。格式為: abc@yahoo.com.tw。多筆接收人時，請以半形逗點隔開( , )，abc@yahoo.com.tw,def@yahoo.com.tw。</param>
        /// <returns>true:傳送成功；false:傳送失敗</returns>
        public bool AllUserSend(string subject, string content, string mailto)
        {
            bool success = false;
            try
            {
