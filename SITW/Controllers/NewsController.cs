...
namespace SITW.Controllers
{
    ...
    public class NewsController : Controller
    {
        ...
        {
            var allnews = new List<placard>();
            var news = this.newsRepository.GetAll().Where(x=>x.p_class != 4).OrderByDescending(x => x.real_time).ToPagedList(1, 10);
            var news4 = this.newsRepository.GetAll().Where(x => x.p_class == 4).OrderByDescending(x => x.real_time).ToPagedList(1, 10);

            allnews.AddRange(news);
            allnews.AddRange(news4);
            allnews = allnews.OrderByDescending(x => x.real_time).ToList();
            List<NewsViewModel> newsList = new List<NewsViewModel>();
            DateTime dt = DateTime.Now;
            foreach (var ns in allnews)
            {
                NewsViewModel newsm = new NewsViewModel();
                newsm.Id = ns.Id;
                newsm.title = ns.title;               
