...
namespace SIT.Controllers
{
    ...
    public class SettingController : Controller
    {
        ...
        public async System.Threading.Tasks.Task<ActionResult> AutoIndex()
        {
            SettingListDto gameb = await new SttingRepository().GetTopicSettingList();
            SettingListDto gameblol = new SettingListDto();
            SettingListDto gamebdota2 = new SettingListDto();
            SettingListDto gamebcsgo = new SettingListDto();
            gameblol.topicsettingList = gameb.topicsettingList.Where(x => x.gametype == 1 && x.Model == 3).ToList();
            gamebdota2.topicsettingList = gameb.topicsettingList.Where(x => x.gametype == 4 && x.Model == 3).ToList();
            gamebcsgo.topicsettingList = gameb.topicsettingList.Where(x => x.gametype == 2 && x.Model == 3).ToList();
            ViewData["lolList"] = gameblol;
            ViewData["Dota2List"] = gamebdota2;
            ViewData["CSGOList"] = gamebcsgo;
            return View();
        ...
        public ActionResult _AutoCSGO(SettingListDto stting)
        {
        ...
        [Authorize(Roles = "Admin")]
        [HttpPost]
        public async System.Threading.Tasks.Task<ActionResult> _AutoPayPost(SettingListDto stting)
        {
            try
            {

                stting = await new DgonHonusRepository().GetSetting(stting);
                return PartialView("_AutoCSGO", stting);
