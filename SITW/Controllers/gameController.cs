    ...
    public class gameController : Controller
    {
        ...
        public async System.Threading.Tasks.Task<ActionResult> AdminIndex()
        {
            ...
            foreach (var gp in gpList)
            {
                ...
                gpvm.TeamB = teamlist.Where(p => p.sn == gp.TeamBSn).FirstOrDefault();
                gpvm.endguess = 1;
                gpvm.PlayGame = new cfgPlayGameRepository().get((int)gp.PlayGameSn);
        ...
        public async System.Threading.Tasks.Task<ActionResult> Create(GamePostViewModel gpvm)
        {
            try
            {
                ...
                if (gpvm.TeamList[0].name != null)
                {
                    ...
                    if (gpvm.gamepost.PlayGameSn == 2 && gpvm.TeamList[0].imageURL == null)
                        gpvm.TeamList[0].imageURL = "https://storage.googleapis.com/funbet-180608.appspot.com/OtherIMG/cogodef.png";
                ...
                if (gpvm.TeamList[1].name != null)
                {
                    ...
                    if (gpvm.gamepost.PlayGameSn == 2 && gpvm.TeamList[1].imageURL == null)
                        gpvm.TeamList[1].imageURL = "https://storage.googleapis.com/funbet-180608.appspot.com/OtherIMG/cogodef.png";
                ...
                gp.AutoSn = gpvm.gamepost.AutoSn;
        ...
        public async System.Threading.Tasks.Task<bool> deleteClose(int id)
        {
            try
            {
                ...
                if (1 == 1 || game.userId == User.Identity.GetUserId())
                {
                    ...
                    game = await _games.GetGameDetail(sbr.gameSn);
                    new GamePostsRepository().DeleteGamePost(game.sn);
                    new SignalRHelper().UpdateTopic(game, encryptedKey, game.md5GameSn);
                    return bresult;
                }
                else
        ...
        [AllowAnonymous]
        [HttpPost]
        public async Task<ActionResult> PostUpdateGameAsync(int id)
        {
            try
            {
                gameDto game = null;
                //GamesRepository _games = new GamesRepository();
                //game = await _games.GetGameDetail(id);
                 List<gameDto> gameList = await new GamesRepository().reflashGameListnoAsync();
                game = gameList.Where(x => x.sn == id).FirstOrDefault();
                //更新緩存
