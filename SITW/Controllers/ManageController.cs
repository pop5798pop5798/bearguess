...
namespace SITW.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        //
        // GET: /Manage/Index
        public async Task<ActionResult> Index(ManageMessageId? message,string RM)
        {
            ViewBag.StatusMessage =
                ...
                : message == ManageMessageId.SendEmailConfirmSuccess ? "已送出驗證信件，請檢查您的信箱。"
                : message == ManageMessageId.EmailConfirmSuccess ? "信箱驗證成功。"
                : message == ManageMessageId.RemoveLoginSuccess ? "已移除外部登入。"
                : message == ManageMessageId.AddLoginSuccess ? "外部登入綁定成功。"
                : "";

            if (!User.Identity.GetEmailConfirmed() && !User.Identity.GetPhoneNumberConfirmed())
            {
                ViewBag.errorMessage = "Email以及手機驗證尚未驗證，將無法參與所有競猜活動";
            }
            else if (!User.Identity.GetEmailConfirmed())
            {
                ViewBag.errorMessage = "Email尚未驗證，將無法參與所有競猜活動";
            }
            else if (!User.Identity.GetPhoneNumberConfirmed())
            {
                ViewBag.errorMessage = "手機尚未驗證，將無法參與所有競猜活動";
            }
            else {
           ...
           /* ViewBag.Log = Log;*/
            ...
            ViewBag.RMessage = (RM != "") ? RM : "";
            return View(model);
        }

        public async Task<ActionResult> GameIndex(ManageMessageId? message)
        {
            ViewBag.StatusMessage =
                message == ManageMessageId.ChangePasswordSuccess ? "已變更您的密碼。"
                : message == ManageMessageId.SetPasswordSuccess ? "已設定您的密碼。"
