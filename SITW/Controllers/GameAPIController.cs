...
namespace SIT.Controllers
{
    ...
    public class GameAPIController : Controller
    {
        ...
        [Authorize(Roles = "Admin")]
        public async System.Threading.Tasks.Task<ActionResult> CSGOUpcoming(int? id)
        {


            List<CSGOUpcoming.RootObject> lolupcoming = await new CSGORepository().GetCSGOSUpcoming((int)id);
        ...
        [Authorize(Roles = "Admin")]
        public async System.Threading.Tasks.Task<ActionResult> CSGOLeaguesUpcoming()
        {


            List<CSGOLeaguesUpcoming.RootObject> csgoupcoming = await new CSGORepository().GetCSGOLeaguesUpcoming();

            DateTime FirstDay = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
            var csgoupcomingDy = csgoupcoming.Where(x=>x.series.Count != 0 && x.series.Last().begin_at > FirstDay).OrderBy(x => x.modified_at).ToList();

            return View(csgoupcomingDy);
        }

        [Authorize(Roles = "Admin")]
        public async System.Threading.Tasks.Task<ActionResult> CSGOSeriesUpcoming()
        {

            List<CSGOSeriesUpcoming.RootObject> csgoall = new List<CSGOSeriesUpcoming.RootObject>();
            List<CSGOSeriesUpcoming.RootObject> csgoupcoming = await new CSGORepository().GetCSGOSeriessUpcoming();
            List<CSGOSeriesUpcoming.RootObject> csgorunning = await new CSGORepository().GetCSGOSeriessRunning();
            csgorunning = csgorunning.OrderByDescending(x => x.begin_at).ToList();
            csgoupcoming.AddRange(csgorunning);
            /* DateTime FirstDay = new DateTime(DateTime.Now.Year, DateTime.Now.Month, 1);
             var csgoupcomingDy = csgoupcoming.Where(x => x.series.Count != 0 && x.series.Last().begin_at > FirstDay).OrderBy(x => x.modified_at).ToList();*/
            csgoall.AddRange(csgoupcoming);
            //csgoall.AddRange(csgorunning);
            return View(csgoall);
        }
