	...
	var Sence=window.Sence=Class.extend({
		...
		changeSense:function(number){
			...
			if(g.senceNumber==0){
                ...
                g.fishArr = [];
				//添加背景音乐
                g.bg_music.src ='../Content/Music/fishjoy/bg.ogg';
		...
		show:function(){
			if(g.senceNumber==0){
                ...
                if (standard > Math.random()) {
                        var fish;
						if(decoration<0){
							fish=new Fish(_.random(1,5), g.canvas.width+50,rnd(0,g.canvas.height),rnd(91,269));
						}else{
							fish=new Fish(_.random(1,5), -50,rnd(0,g.canvas.height),rnd(-89,89));
						}
						g.fishArr.push(fish);
					}
				...
				g.ctx.fillText("彈藥庫："+g.bulletCount+"發",638,67);
				//炮弹出现
				...
				switch(g.senceNumber){
					...
					case 1:
						...
						if(x > cannonPlusX && x < cannonPlusX + 23 && y >  cannonPlusY && y < cannonPlusY + 18){
							...
							//添加炮弹发射的声音文件
							var oM=new Audio();
                            oM.src ="../Content/Music/fishjoy/bullet.ogg";
							oM.play();
							//记录鼠标点击的位置
							var cx=event.clientX-g.canvas.offsetLeft- g.cannon.x;
							var cy= g.cannon.y-(event.clientY+g.canvas.offsetTop);
							//计算角度，tan为临边比对边
		...
		fishAndBulletIsEmit:function(){
			...
			_.each(g.fishArr,function(fish){
				_.each(g.arrBullet,function(bullet){
					...
					if(fish.isCrash(writeBullet)){
						self.emitFishArr.push({"x":fish.x,"y":fish.y,"rotate":fish.rotate,"type":fish.type,"score":fish.score});
			...
			_.each(self.emitFishArr,function(emitFish){
				...
				//碰撞的时候添加金币的声音文件
				var oM=new Audio();
                oM.src ="../Content/Music/fishjoy/coin.ogg";
				oM.play();
                ...
                console.log("當前魚的分數：" + score);
                //傳入後端
                userpost(g.user,score);
				//改变总分数
				g.score+=score;
				console.log("總分數："+g.score);
				//将鱼从数actors数组中移除
