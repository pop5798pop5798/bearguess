(function(){
	var Game=window.Game=Class.extend({
		init:function(id,resourseUrl,userdata){
		...
		loadResource:function(){
			...
			self.ctx.fillText("圖片正在加載中 .....",300,self.canvas.height*(1-0.618));
			//原生js Ajax请求
				...
				if(xhr.status >= 200 && xhr.status < 300 || xhr.status == 304){
					...
					for(var k in self.RobjectTxt){
						...
						self.Robject[k].onload=function(){
							...
							self.ctx.fillText("圖片已加載到 "+count+"/"+imageAmount,300,self.canvas.height*(1-0.618));
							//判断是否加载完毕
		...
		start:function(){
			...
			this.timer=setInterval(function(){
                ...
                m = m < 10 ? "0" + m : m;
                mm = mm < 10 ? "0" + mm : mm;
                g.ctx.fillText("遊戲時間: "+ h + " : " + m + " : " + mm, 250, 70);
			}, 40);
            self = this;
